...
def beregn_gpx(gpx, post_data, bane_gpx, navn_gpx):
    ...
    def find_result(correct, reduced_user_inputs, var_i):
        ...
        straekket = reduced_user_inputs[:max_idx]
        punkt = []
        for punkter in straekket:
            d = datetime.fromisoformat(str(punkter['time']))
            timeconv = d.strftime('%H:%M:%S')
            punkt.append({
                        'latitude': punkter['latitude'],
                        'longitude': punkter['longitude'],
                        'time': timeconv,
                        'coor': [punkter['latitude'], punkter['longitude']]
                    })
        mindste_afstand['straek_spor'] = punkt
    ...
    for i, correct in enumerate(correct_inputs):
        ...
        #postpaseret.append(result['straek_spor'])
    ...
    for a, post in enumerate(postpaseret):
        ...
        if y == 1:
            ...
            minAfstandItem['straek_spor'] = post['straek_spor']
            afstandtilMeter = 0
...
def map_create(gpx_spor, post_data, resultat_data, kort_data, bane, path):
    ...
    folium.PolyLine(coordinates, weight=5, color='red').add_to(m)
    #m.add_children(plugins.ImageOverlay(kort, opacity=0.8, bounds =[[min_lat, min_lon], [max_lat, max_lon]]))
    return m

def map_create_flere(gpx_spor, post_data, kort_data, bane, path):
    ...
    for a, post in enumerate(post_data):
        sted[a] = a
        sted['latitude'] = post['coor'][0]
        sted['longtitude'] = post['coor'][1]
        latitude = post['coor'][0]
    ...
    '''# Posternes faktiske placering'''
    #for _, row in enumerate(poster[bane]):
    for _, row in enumerate(post_data):
        statuscolor='green',
        folium.CircleMarker(
                #location= [row['latitude'],row['longitude']],
                location= row['coor'],
    ...
    '''# selve sporets visning'''
    for i, spor in enumerate(gpx_spor):
        #if type(spor) == list:
        #    spor = gpx_spor
        #elif type(gpx_spor) == str:
        #    spor = gpx_spor
        ...
        if i == 0:
            ...
            spor_color = 'green'
        folium.PolyLine(coordinates, weight=5, color=spor_color).add_to(m)
        #m.add_children(plugins.ImageOverlay(kort, opacity=0.8, bounds =[[min_lat, min_lon], [max_lat, max_lon]]))
    return m
